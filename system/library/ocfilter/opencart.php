...
final class OpenCart {  
  ...
  public function isAdminLogged() {    
    if (is_null($this->admin_logged)) {
      // Admin status for debug/quick settings and color editor
      if (class_exists('\Cart\User')) {
        $user = new \Cart\User($this->registry);
      } else if (class_exists('\User')) {
  ...
  public function getThemeFile($path) {
    ...
    if ($this->version < 22) {
      ...
      if (is_file(DIR_TEMPLATE . $this->config->get('theme_' . $this->config->get('config_theme') . '_directory') . '/' . $path)) {
        $file = 'catalog/view/theme/' . $this->config->get('theme_' . $this->config->get('config_theme') . '_directory') . '/' . $path;
      }
    }    
