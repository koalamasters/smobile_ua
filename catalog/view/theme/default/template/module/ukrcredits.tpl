	...
	<div id="prop<?php echo $credit['type']; ?>" class="proposition">
		...
		<div class="prop_info">
			<div class="prop_calc">
				...
				<div id="termSlider<?php echo $credit['type']; ?>" <?php echo ($credit['type']=='AB'||$credit['type']=='SB'||$credit['type']=='PL')?'style="display:none"':''; ?>>
...
<?php foreach ($credits_data as $credit) { ?>
	...
	'termMin<?php echo $credit['type']; ?>': <?php echo ($credit['type']=='MB' || $credit['type']=='PU')?2:(($credit['type']=='AB')?3:((($credit['type']=='PL')&&(isset($ukrcredits_pl_data)&&$ukrcredits_pl_data))?($ukrcredits_pl_data['pack0']['count']-1):((($credit['type']=='SB')&&(isset($ukrcredits_sb_data)&&$ukrcredits_sb_data))?key($ukrcredits_sb_data):1))); ?>,
	'termSelected<?php echo $credit['type']; ?>': <?php echo $credit['partsCountSel']?$credit['partsCountSel']:($credit['type']=='MB'?2:(($credit['type']=='AB')?3:((isset($ukrcredits_pl_data)&&$ukrcredits_pl_data)?($ukrcredits_pl_data['pack0']['count']-1):((($credit['type']=='SB')&&(isset($ukrcredits_sb_data)&&$ukrcredits_sb_data))?key($ukrcredits_sb_data):1)))); ?>,
	<?php } ?>
<?php } ?> 
};
...
UC_CALCULATOR = (function () {
    ...
    uc.calculatePhys = function (paymentsCount, price) {
		...
		<?php if ($ukrcredits_setting['ab_markup_type'] == 'custom') { ?>
		var commissionsSB = $.parseJSON('<?php echo json_encode($ukrcredits_setting['sb_markup_custom_sb']); ?>');
		markupSB = Number(commissionsSB[paymentsCount-1]) + Number(<?php echo $ukrcredits_setting['sb_markup_acquiring']; ?>);
		price_sb = price + price * (markupSB / 100);
		<?php } else {?>
		price_sb = price;
		<?php } ?>
		
		<?php if ($ukrcredits_setting['aa_markup_type'] == 'custom') { ?>
		var commissionsAA = $.parseJSON('<?php echo json_encode($ukrcredits_setting['aa_markup_custom_AA']); ?>');
		markupAA = Number(commissionsAA[paymentsCount-1]) + Number(<?php echo $ukrcredits_setting['aa_markup_acquiring']; ?>);
		price_aa = price + price * (markupAA / 100);
		<?php } else {?>
		price_aa = price;
		<?php } ?>

		<?php if ($ukrcredits_setting['pl_markup_type'] == 'custom') { ?>
		var commissionsPL = $.parseJSON('<?php echo json_encode($ukrcredits_setting['pl_markup_custom_PL']); ?>');
		markupPL = Number(commissionsPL[paymentsCount-1]) + Number(<?php echo $ukrcredits_setting['pl_markup_acquiring']; ?>);
		price_pl = price + price * (markupPL / 100);
		<?php } else {?>
		price_pl = price;
		<?php } ?>
		...
		pl_data_markupPL = Number((pl_data[paymentsCount-1]));	
		<?php } else { ?>	
		pl_data_markupPL = 1;
		<?php } ?>
		
		<?php if ($ukrcredits_setting['pu_markup_type'] == 'custom') { ?>
		var commissionsPP = $.parseJSON('<?php echo json_encode($ukrcredits_setting['pu_markup_custom_PU']); ?>');
		markupPU = Number(commissionsAA[paymentsCount-1]) + Number(<?php echo $ukrcredits_setting['pu_markup_acquiring']; ?>);
		price_pu = price + price * (markupPU / 100);
		<?php } else {?>
		price_pu = price;
		<?php } ?>
		...
		var ab = price_ab / (paymentsCount-1);
		var sb = price_sb / (paymentsCount-1);
		var aa = price_aa / paymentsCount;
		var pl = price_pl / pl_data_markupPL / paymentsCount;
		var pu = price_pu / paymentsCount;
		
		if ((paymentsCount-1) == 3) {
			var sb = (price_sb + 500) / 3;
			price_sb = price_sb + 500;
		}
		if ((paymentsCount-1) == 5) {
			var sb = (price_sb+(price_sb*0.0527+250))/5;
			price_sb = price_sb+(price_sb*0.0527+250);
		}		
		if ((paymentsCount-1) == 10) {
			var sb = (price_sb+(price_sb*0.111+500))/10;
			price_sb = price_sb+(price_sb*0.111+500);
		}	
		if ((paymentsCount-1) == 12) {
			var sb = (price_sb+(price_sb*0.1364+600))/12;
			price_sb = price_sb+(price_sb*0.1364+600);
		}	
        return ({
			...
			aaValue: aa.toFixed(2),
			plValue: pl.toFixed(2),
			puValue: pu.toFixed(2),
