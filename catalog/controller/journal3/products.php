	...
	protected function beforeRender() {
		...
		foreach ($this->settings['items'] as $key => $item) {
            ...
            foreach ($products as $product_id => &$product){
                $is_new = $this->model_catalog_product->getNewStatus($product_id);

                $date_added = strtotime($this->model_catalog_product->getDateCreate($product_id));
                $current_time = time();
