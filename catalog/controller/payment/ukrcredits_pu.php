<?php
class ControllerPaymentUkrcreditspu extends Controller {
    ...
    public function index() {
		...
		$data['text_success'] = $this->language->get('text_success_pu');
		...
		foreach ($this->cart->getProducts() as $cart) {
			...
			if ($privat_query->row) {
				if ($privat_query->row['partscount_pu'] <= $partsCount && $privat_query->row['partscount_pu'] !=0) {
					$partsCount = (int)$privat_query->row['partscount_pu'];
				}
			}
		}
		if ($partsCount == 24) {
			$partsCount = $setting['pu_pq'];
		}
        ...
        $data['action'] = $this->url->link('payment/ukrcredits_pu/senddatadeal', '', 'SSL');	
		
		$data['credit'] = array(
			'type' => $setting['pu_merchantType'],
			'name' => $this->language->get('text_title_'.mb_strtolower($setting['pu_merchantType'])),
			'partsCount' => $partsCount,
			'price' => $data['total']
		);
		
		if (isset($this->session->data['ukrcredits_pu_sel'])) {
			$data['credit']['partsCountSel'] = $this->session->data['ukrcredits_pu_sel'];
		} else {
	...
	public function setUkrcreditsType(){
		...
		$this->language->load($dir.'/ukrcredits');
		$this->session->data['payment_method']['title'] = $this->language->get('text_title_pu');
		$this->session->data['payment_method']['code'] = 'ukrcredits_pu';
		setcookie('payment_method', 'ukrcredits_pu', time() + 60 * 60 * 24 * 30);
		$this->session->data['ukrcredits_pu_sel'] = $this->request->post['partsCount'];
        $json['success'] = TRUE;
    ...
    public function senddatadeal(){
        ...
        if ($order_info) {
			...
			$data_deal['channel_type'] = 'INTERNET';
			$data_deal['flow']['type'] = 'DIGITAL_SF';
			$data_deal['partner_name'] = $setting['pu_shop_name'];
			
			$data_deal['customer']['phone'] = str_replace(['+','(', ')', '-', ' '], '', $order_info['telephone']);
			if (substr($data_deal['customer']['phone'], 0, 1) == 0) {
			$data_deal['customer']['phone'] = '+38' . $data_deal['customer']['phone'];
			} else if (substr($data_deal['customer']['phone'], 0, 2) == 80) {
			$data_deal['customer']['phone'] = '+3' . $data_deal['customer']['phone'];
			} else {
			$data_deal['customer']['phone'] = '+' . $data_deal['customer']['phone'];	
			}
				...
				if (($total['code'] != 'sub_total') && ($total['code'] != 'total')) {
					if ($total['value'] > 0) {
						$products[] = array(
							...
							'count' => 1,
							'amount'    => $this->currency->format($total['value'], 'UAH', '', false)
						);
            ...
            foreach ($this->cart->getProducts() as $product) {
				if (($this->currency->format($product['price'], 'UAH', '', false) - $minus) <= 0) {
					...
					$products[] = array(
						...
						'amount'    => $this->currency->format($product['price'], 'UAH', '', false)
					);
			...
			$data_deal['invoices'][] = array(
				'date' 				=> date($this->language->get('Y-m-d'), strtotime($order_info['date_added'])),
				'invoice_number' 	=> $order_info['order_id'],
				'goods'				=> $products,
				'total_amount' 		=> $sumtotal
			);

			$data_deal['credit_request'] = array(
				'term'  	=> $this->request->post['partsCount']-1,
				'amount'	=> $sumtotal
			);
			...
			$requestToken['client_id'] = 'EXT_OIC';
			$requestToken['username'] = $setting['pu_shop_id'];
			$requestToken['password'] = $setting['pu_shop_password'];
			$requestToken['grant_type'] = 'password';			
			
			$responseToken = $this->model_module_ukrcredits->curlPostWithDataPUtoken($requestTokenurl, $requestToken);

			if(is_array($responseToken)){
				if(isset($responseToken['access_token']) && $responseToken['access_token']) {
					...
					$responseResDeal = $this->model_module_ukrcredits->curlPostWithDataPU($url, $responseToken, json_encode($data_deal));

					if(is_array($responseResDeal)){
						if(isset($responseResDeal['id']) && $responseResDeal['id']) {
							$paymenttype = 'PU';
							$comment = $this->language->get('text_status_IN_PROCESSING') . ($data_deal['credit_request']['term']) . ' ' . $this->language->get('text_mounth');
							...
							$responseResDeal['state'] = 'SUCCESS';
						} else {
							$this->log->write('ukrcredits_pu получен отказ ' . $responseResDeal['errorMessage']);
						}
					}
    ...
    public function callback() {
		...
		$order_mono = $this->model_checkout_order->getOrderMb($requestArr['cap_id']);
		$order_info = $this->model_checkout_order->getOrder($order_mono['order_id']);
		
		$this->log->write('ukrcredits_pu - получен колбэк');
		$this->log->write($requestPostRaw);

        if ($order_info) {        
            if ($requestArr['state']) {
				$comment = $requestArr['state'];
				...
				$this->model_checkout_order->updateUkrcreditsOrderPrivat($order_info['order_id'], $requestArr['state']);
				if (version_compare(VERSION,'2.0','>=')) {
                ...
                $this->log->write('ukrcredits_pu :: Статус не получен!  ORDER_ID:'.$order_id);
				$this->log->write($requestArr);
            } 
        }
    }
}