<?php
class ControllerPaymentUkrcreditsAa extends Controller {
    ...
    public function index() {
		...
		$data['text_success'] = $this->language->get('text_success_aa');
		...
		foreach ($this->cart->getProducts() as $cart) {
			...
			if ($privat_query->row) {
				if ($privat_query->row['partscount_aa'] <= $partsCount && $privat_query->row['partscount_aa'] !=0) {
					$partsCount = (int)$privat_query->row['partscount_aa'];
				}
			}
		}
		if ($partsCount == 24) {
			$partsCount = $setting['aa_pq'];
		}
        ...
        $data['action'] = $this->url->link('payment/ukrcredits_aa/senddatadeal', '', 'SSL');	
		
		$data['credit'] = array(
			'type' => $setting['aa_merchantType'],
			'name' => $this->language->get('text_title_'.mb_strtolower($setting['aa_merchantType'])),
			'partsCount' => $partsCount,
			'price' => $data['total']
		);
		
		if (isset($this->session->data['ukrcredits_aa_sel'])) {
			$data['credit']['partsCountSel'] = $this->session->data['ukrcredits_aa_sel'];
		} else {
	...
	public function setUkrcreditsType(){
		...
		$this->language->load($dir.'/ukrcredits');
		$this->session->data['payment_method']['title'] = $this->language->get('text_title_aa');
		$this->session->data['payment_method']['code'] = 'ukrcredits_aa';
		setcookie('payment_method', 'ukrcredits_aa', time() + 60 * 60 * 24 * 30);
		$this->session->data['ukrcredits_aa_sel'] = $this->request->post['partsCount'];
        $json['success'] = TRUE;
    ...
    public function senddatadeal(){
        ...
        if ($order_info) {
			...
			$data_deal['result_callback'] = $this->url->link('payment/ukrcredits_aa/callback', '', 'SSL');;

			$requestDial = json_encode($data_deal);
			
			$signature = base64_encode(hash_hmac("sha256", $requestDial, $setting['aa_shop_password'], true));

			$url = 'https://' . ($setting['aa_mode']?'midgard.a-bank.com.ua/partpay/common':'midgard.a-bank.com.ua/partpay/common') . '/newLoan';

			$responseResDeal = $this->model_module_ukrcredits->curlPostWithDataAA($url, $requestDial, $signature);
		
			if(is_array($responseResDeal)){
				if(isset($responseResDeal['order_id']) && $responseResDeal['order_id']) {
					$this->log->write('ukrcredits_aa получен ответ ' . $responseResDeal['order_id']);
					...
					$this->log->write('ukrcredits_aa получен отказ ' . $responseResDeal['message']);
				}
			}
            ...
            if ($requestArr['state']) {
                switch($requestArr['state']) {
                  case 'SUCCESS':
					...
					$this->log->write('ukrcredits_aa :: PAYMENT SUCCESS!  ORDER_ID:'. $order_info['order_id'] .' MESSAGE:'. $requestArr['message']);
					break;
                  case 'FAIL':
					switch($requestArr['order_sub_state']) {
					...
					$this->log->write('ukrcredits_aa :: PAYMENT FAIL!  ORDER_ID:'. $order_info['order_id'] .' MESSAGE:'. $requestArr['result']);
					break;  
                  case 'IN_PROCESS':
					switch($requestArr['order_sub_state']) {
					...
					$this->log->write('ukrcredits_aa :: PAYMENT IN_PROCESS!  ORDER_ID:'. $order_info['order_id'] .' MESSAGE:'. $requestArr['message']);
					break;
                }
                ...
                $this->log->write('ukrcredits_aa :: Статус не получен!  ORDER_ID:'.$order_id .' RECEIVED:'. $requestArr['signature']);
            } 
        }
    }
}