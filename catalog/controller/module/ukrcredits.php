...
class ControllerModuleUkrcredits extends Controller {
	...
	public function index() {
		...
		$data['text_parts'] = $this->language->get('parts');
		...
		if ($product_info) {
			...
			if ($data['credits_data']['pp'] || $data['credits_data']['ii'] || $data['credits_data']['mb'] || $data['credits_data']['ab'] || $data['credits_data']['sb'] || $data['credits_data']['aa'] || $data['credits_data']['pl'] || $data['credits_data']['pu']) {
				$status = true;
				$pricemin = array();
				...
				if (empty($pricemin)) $pricemin = array(0);
	...
	public function loadpopup(){
		...
		$data['ukrcredits_sb_data'] = $ukrcredits_setting['sb_id_data']; 
