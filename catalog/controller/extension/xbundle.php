<?php
class ControllerExtensionxbundle extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$filter_data = array(
			'sort'  => 'p.date_added',
			'order' => 'DESC',
			'start' => ($page - 1) * $xbundle_imit,
			'limit' => $xbundle_imit
		);
		
		$xbundle_total = $this->model_extension_xbundle->getTotalbundles();
				  ...
				  $product_info = $this->model_catalog_product->getProduct($product_id);
				   if($product_info){
					...
					$wproducts[]=array(
						...
						'minimum'	  => ($product_info['minimum']>1) ? $product_info['minimum'] : 1,
						'thumb'       => $image,
		...
		$pagination->total = $xbundle_total;
		$pagination->page = $page;
		$pagination->limit = $xbundle_imit;
		$pagination->url = $this->url->link('extension/xbundle', '&page={page}',true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($xbundle_total) ? (($page - 1) * $xbundle_imit) + 1 : 0, ((($page - 1) * $xbundle_imit) > ($xbundle_total - $xbundle_imit)) ? $xbundle_total : ((($page - 1) * $xbundle_imit) + $xbundle_imit), $xbundle_total, ceil($xbundle_total / $xbundle_imit));
		...
		$data['header'] = $this->load->controller('common/header');
			...
			$this->response->setOutput($this->load->view('extension/xbundle', $data));
	}
		...
		if(isset($this->request->get['bundle_id'])){
			...
			if($xbundles['product']){
				...
				foreach($xbundles['product'] as $product_id){
					...
					if ($product_info){
						if(!$json){
							$this->cart->add($product_id, ($product_info['minimum']>1) ? $product_info['minimum'] : 1, $option, 0);
							$success = true;
						}
					}
				}
