    ...
    public function index($setting) {
        ...
        foreach ($results as $key => $result) {
            ...
            if (isset($image[(int)$this->config->get('config_language_id')]) && is_file(DIR_IMAGE.$image[(int)$this->config->get('config_language_id')])) {
                ...
                $image2 = ($data['full_img'] && isset($image_width) && isset($image_height))
                    ? $this->model_tool_image->resize($image[(int)$this->config->get('config_language_id')], $image_width, $image_height)
                    : $this->model_tool_image->resize($image[(int)$this->config->get('config_language_id')], $setting['width'], $setting['height']);


                $mobile_image2 = ($data['full_img'] && isset($mobile_image_width) && isset($mobile_image_height)) ? $this->model_tool_image->resize($mobile_image[(int)$this->config->get('config_language_id')], $mobile_image_width, $mobile_image_height) : false;
