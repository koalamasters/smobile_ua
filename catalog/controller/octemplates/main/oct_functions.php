...
class ControllerOCTemplatesMainOCTFunctions extends Controller {
    ...
    public function mobileContacts() {
        if (isset($this->request->server['HTTP_X_REQUESTED_WITH']) && !empty($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            ...
            if (isset($oct_showcase_data['mobile_menu']['address']) && $oct_showcase_data['mobile_menu']['address']) {
                if (isset($oct_showcase_data['contact_address'][(int)$this->config->get('config_language_id')]) && !empty($oct_showcase_data['contact_address'][(int)$this->config->get('config_language_id')])) {
                ...
                if (isset($oct_showcase_data['contact_map']) && !empty($oct_showcase_data['contact_map'])) {
    ...
    public function OctDateTime($array) {
        $data = array_merge([], $this->language->load('octemplates/oct_showcase'));
