...
class ControllerOCTemplatesBlogOCTBlogCategory extends Controller {
        ...
        if ($blog_category_info) {
            ...
            */
            ...
            foreach ($results as $result) {
                ...
                $article_date_added = $this->load->controller('octemplates/main/oct_functions/OctDateTime', array($result['date_added'], 0));

                $date = DateTime::createFromFormat('Y-m-d H:i:s', $result['date_added']);
                ...
                $data['articles'][] = [
                    ...
                    'date_added' 			=> $article_date_added,
                    'date_added_2'          => $formattedDate,
            ...
            $sort_query_param = isset($this->request->get['sort']) ? $this->request->get['sort'] : '';
            $order_query_param = isset($this->request->get['order']) ? $this->request->get['order'] : '';
            if (!$sort_query_param && !$order_query_param) {
                $additional_params = '';
            } else {
                $additional_params = 'sort=' . $sort_query_param . '&order=' . $order_query_param;
            }
            ...
            $pagination->url = $this->url->link('octemplates/blog/oct_blogcategory', 'blog_path=' . $this->request->get['blog_path'] . $url . '&page={page}&' . $additional_params, true);
            ...
            $current_category_id = isset($this->request->get['blog_path']) ? (int)$this->request->get['blog_path'] : 1;
            $data['sorts'][] = [
                'text' => $this->language->get('sort_default'),
                'value' => 'a.date_added-DESC',
                'href' => $this->url->link('octemplates/blog/oct_blogcategory', '&blog_path=' . $current_category_id . '&sort=a.date_added&order=DESC' . $url)
            ];
            $data['sorts'][] = [
                'text' => $this->language->get('sort_popular'),
                'value' => 'a.viewed-DESC',
                'href' => $this->url->link('octemplates/blog/oct_blogcategory', '&blog_path=' . $current_category_id . '&sort=a.viewed&order=DESC' . $url)
            ];


            if (isset($this->request->get['dev'])) {
                $this->response->setOutput($this->load->view('octemplates/blog/oct_blogcategory', $data));
            } else {
                $this->response->setOutput($this->load->view('octemplates/blog/oct_blogcategory_new', $data));
            }
    ...
    private function convertDate($ukrainian_date) {
        // Ukrainian months mapping
        $months = [
        ...
        // Replace Ukrainian month with English equivalent
        foreach ($months as $ukr => $eng) {
            if (strpos($ukrainian_date, $ukr) !== false) {
                $ukrainian_date = str_replace($ukr, $eng, $ukrainian_date);
                break;
            }
        }

        // Convert to DateTime
        $date = DateTime::createFromFormat('j F Y', $ukrainian_date);
