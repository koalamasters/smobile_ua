...
class ControllerOCTemplatesEventsMenuModule extends Controller {
    ...
    public function index(&$route, &$data) {
        ...
        $categories_icon = isset($oct_showcase_data['megamenu']['icon']) ? true : false;
        ...
        if (!$result_all_categories) {
            $result_all_categories = $this->fetchCategories($categories_icon, $oct_showcase_data);
    ...
    private function fetchCategories() {
        $categories_icon = isset($this->config->get('theme_oct_showcase_data')['megamenu']['icon']) ? true : false;
        ...
        foreach ($categories as $category) {
            if ($category['top']) {
                ...
                $result_all_categories[] = array(
                    ...
                    'column'   => 1,
                    'oct_pages' => isset($this->config->get('theme_oct_showcase_data')['megamenu']['page']) ? unserialize($category['page_group_links']) : [],
                    'oct_image'     => $oct_image,
                    'width'     => 32,
                    'height'     => 32,
    ...
    private function fetchChildren($parent_category) {
        ...
        foreach ($children as $child) {
            ...
            $children_data[] = array(
                ...
                'oct_pages' => isset($this->config->get('theme_oct_showcase_data')['megamenu']['page']) ? unserialize($child['page_group_links']) : [],
