...
class ModelToolRemarketing extends Model {
	...
	public function getRemarketingFooter() { 
		...
		$esputnik_id = $this->config->get('remarketing_esputnik_id') == 'id' ? 'product_id' : 'model';
		  
		$google_ids = []; $facebook_ids = []; $tiktok_ids = []; $snapchat_ids = [];	$uet_ids = [];
		$total_value = 0;
		$google_page = false; $facebook_page = false; $tiktok_page = false;	$google_reviews_page = false;
		...
		switch ($route) {
				...
				if (!empty($this->session->data['remarketing_register'])) {
					...
					if ($this->config->get('remarketing_facebook_status')) {
						if ($this->config->get('remarketing_facebook_pixel_status')) {
							$output .= "<script>$(document).ready(function(){if(typeof fbq != 'undefined'){fbq('track', 'CompleteRegistration', {eventID: '" . $fb_event_id . "'})}});</script>\n";
						}
			...
			case 'product/product':
				...
				if ($this->config->get('remarketing_esputnik_status') && $this->config->get('remarketing_esputnik_webtracking_status') && $this->config->get('remarketing_esputnik_webtracking_identifier')) {
					$output .= "<script>if (typeof eS != 'undefined') { eS('sendEvent', 'ProductPage', {'ProductPage': {'productKey': '" . $product_info[$esputnik_id] . "', 'price': '" . $ecommerce_total . "', 'isInStock': " . ($product_info['quantity'] > 0 ? 1 : 0) . ", }}, {" . $esputnik_general_info . "});}</script>\n\n";
				} 
				break;					
			case 'checkout/cart':
			...
			case 'checkout/checkout_two_step':
			case 'checkout/buy':
			...
			case 'quickcheckout/checkout':
				...
				$output .= "<script>console.log('%c%s', 'color: #a700ff', 'begin_checkout');</script>";
				break;	
				...
				if (!empty($this->request->cookie['remarketing_order_id']) || !empty($this->session->data['order_id']) || !empty($this->session->data['remarketing_order_id'])) {
					...
					if ($order_info) {
						...
						if ($this->config->get('remarketing_google_ads_identifier')) {
							$conversion_id = $this->config->get('remarketing_google_ads_identifier');
							if (!empty($this->session->data['quick_order']) && $this->config->get('remarketing_google_ads_quick_order_identifier')) {
								$conversion_id = $this->config->get('remarketing_google_ads_quick_order_identifier');
							}
							$output .= '<script>' . "\n";
							$output .= "if (typeof gtag != 'undefined') {" . "\n";
							$output .= 'gtag("event", "conversion", {' . "\n";
							$output .= "'send_to': '" . $conversion_id . "'," . "\n"; 
						...
						if ($this->config->get('remarketing_ecommerce_ga4_status') && !$this->config->get('remarketing_ga4_only_purchase')) {
							$event_name = 'purchase';
							if (!empty($this->session->data['quick_order']) && $this->config->get('remarketing_ecommerce_quick_order_event_name')) {
								$event_name = $this->config->get('remarketing_ecommerce_quick_order_event_name');
							}
							$output .= '<script>' . "\n";
							$output .= "if (typeof gtag != 'undefined') {" . "\n"; 
							$output .= 'gtag("event", "' . $event_name . '", ' . json_encode($order_info['ga4_event']) . ');' . "\n";
							$output .= '}' . "\n";
							$output .= '</script>' . "\n";
						}
						...
						if ($this->config->get('remarketing_esputnik_status') && $this->config->get('remarketing_esputnik_webtracking_status') && $this->config->get('remarketing_esputnik_webtracking_identifier') && isset($this->session->data['remarketing_esputnik_cart_id'])) {
							...
							foreach ($order_info['products'] as $product) {
								$sputnik_products[] = [
									'productKey' => $product[$esputnik_id],
									'price'      => (string)$product['ecommerce_price'], 
						...
						$output .= "<script>console.log('%c%s', 'color: #a700ff', 'order_sent');</script>";
					} 
		...
		if (!empty($this->session->data['remarketing_login'])) {
			if ($this->config->get('remarketing_ecommerce_status')) {
			...
			if ($facebook_page == 'purchase' || $facebook_page == 'initiate') {
				if (!empty($facebook_ids)) {
					...
					if (count($facebook_ids) == 1) {
						...
						if (!empty($facebook_ids[0]['category'])) $output .= "content_category: '" . addslashes($facebook_ids[0]['category']) . "'," . "\n";
					} else {  
						$output .= "contents: [" . "\n";
						foreach ($facebook_ids as $product) {
		...
		if ($google_reviews_page && $this->config->get('remarketing_reviews_status') && $this->config->get('remarketing_google_merchant_identifier') && strpos($reviews_order_email, 'localhost') === false && strpos($reviews_order_email, 'empty') === false) {			
		...
		if ($this->config->get('remarketing_ecommerce_ga4_status') && $this->config->get('remarketing_ecommerce_ga4_selector')) {
			$output .= "<script>$(document).on('click touchstart', '" . $this->config->get('remarketing_ecommerce_ga4_selector') . ", .fm-module-item, .rm-module-item, .sc-module-item, .us-module-item, .ds-module-item', (e) => {" . "\n";
			/* big thanks to @soor ) */
	...
	public function remarketingAddToCart($product_info = [], $quantity = 1, $options = '', $option = []) {
		...
		if ($product_info) {
			...
			if (!empty($options)) $ecommerce_ga4_product['item_variant'] = addslashes($options);
			...
			$json_data['rem_id'] = '21513890285071555250444'; 
			...
			if ($this->config->get('remarketing_esputnik_status') && $this->config->get('remarketing_esputnik_webtracking_status') && $this->config->get('remarketing_esputnik_webtracking_identifier')) {
				...
				foreach ($this->cart->getProducts() as $cart_product) {
					$cart_products[] = [
						'productKey' => (string)$cart_product[$esputnik_id],
						'price'      => (string)$this->currency->format($cart_product['price'], $ecommerce_currency, '', false),
	...
	public function remarketingRemoveFromCart($product_info = [], $quantity = 1) {
		...
		if ($product_info) {
			...
			if (!empty($product_info['manufacturer'])) $ecommerce_ga4_product['item_brand'] = addslashes($product_info['manufacturer']);
			if (!empty($variant)) $ecommerce_ga4_product['item_variant'] = $variant; 
			if (!empty($ga4_categories[0])) $ecommerce_ga4_product['item_category'] = addslashes($ga4_categories[0]);
			if (!empty($ga4_categories[1])) $ecommerce_ga4_product['item_category2'] = addslashes($ga4_categories[1]);
			if (!empty($ga4_categories[2])) $ecommerce_ga4_product['item_category3'] = addslashes($ga4_categories[2]);
			if (!empty($ga4_categories[3])) $ecommerce_ga4_product['item_category4'] = addslashes($ga4_categories[3]);
			...
			if ($this->config->get('remarketing_esputnik_status') && $this->config->get('remarketing_esputnik_webtracking_status') && $this->config->get('remarketing_esputnik_webtracking_identifier')) {
				...
				foreach ($this->cart->getProducts() as $cart_product) {
					$cart_products[] = [
						'productKey' => $this->config->get('remarketing_esputnik_id') == 'id' ? $cart_product['product_id'] : $cart_product['model'],
						'price'      => (string)$this->currency->format($cart_product['price'], $this->config->get('remarketing_ecommerce_currency'), '', false),
	...
	public function remarketingWishlist($product_info = []) {
		...
		if ($product_info) {
			...
			$json_data = [];
			$facebook_product_id = $this->config->get('remarketing_facebook_id') == 'id' ? $product_info['product_id'] : $product_info['model'];
			$ga4_product_id = $this->config->get('remarketing_ecommerce_ga4_id') == 'id' ? $product_info['product_id'] : $product_info['model'];
			$tiktok_product_id = $this->config->get('remarketing_tiktok_id') == 'id' ? $product_info['product_id'] : $product_info['model'];
			$esputnik_id = $this->config->get('remarketing_esputnik_id') == 'id' ? $product_info['product_id'] : $product_info['model'];
			...
			$json_data['ecommerce_ga4_status'] = $this->config->get('remarketing_ecommerce_ga4_status');
			$json_data['esputnik_status'] = $this->config->get('remarketing_esputnik_status') && $this->config->get('remarketing_esputnik_webtracking_status') && $this->config->get('remarketing_esputnik_webtracking_identifier');
			$json_data['s' . rand(14, 98)] = '5515074404250751100';
			$current_price = $product_info['special'] ? $product_info['special'] : $product_info['price'];
			$facebook_price = $this->currency->format($current_price * (float)$this->config->get('remarketing_facebook_ratio'), $this->config->get('remarketing_facebook_currency'), '', false);
			$ecommerce_price = $this->currency->format($current_price * (float)$this->config->get('remarketing_ecommerce_ratio'), $this->config->get('remarketing_ecommerce_currency'), '', false);
			$tiktok_price = $this->currency->format($current_price * (float)$this->config->get('remarketing_tiktok_ratio'), $this->config->get('remarketing_tiktok_currency'), '', false);
			$brand = !empty($product_info['manufacturer']) ? addslashes($product_info['manufacturer']) : '';
			$name = addslashes($product_info['name']);
			...
			$json_data['esputnik_event'] = [
				'AddToWishlist' => [
					'productKey' => $esputnik_id,
					'price'      => (string)$ecommerce_price,
	...
	public function getQuickOrderSuccess($order_id, $send_history = false) {
		...
		$order_info = $this->getOrderRemarketing($order_id);
		if ($order_info) {
			$this->db->query("UPDATE `" . DB_PREFIX . "remarketing_orders` SET `quick_order` = 1 WHERE order_id = '" . (int)$order_id . "'");
			$json['remarketing'] = [];
			$google_products = [];
			$facebook_products = [];
			$tiktok_products = [];
			...
			$json['remarketing']['google_ads_quick_identifier'] = $this->config->get('remarketing_google_ads_quick_order_identifier');
			...
			$json['remarketing']['order_id'] = $order_info['order_id'];
			$json['remarketing']['order_total'] = $order_info['default_total'];
			...
			if ($order_info['products']) {
				...
				foreach ($order_info['products'] as $product) {
					...
					$sputnik_products[] = [
						'productKey' => $this->config->get('remarketing_esputnik_id') == 'id' ? $product['product_id'] : $product['model'],
						'price'      => (string)$product['ecommerce_price'],
						'quantity'   => $product['quantity'],
						'currency'   => $ecommerce_currency
					];
			...
			$quick_order_event_name = $this->config->get('remarketing_ecommerce_quick_order_event_name');
			if (!empty($quick_order_event_name)) {
				$json['remarketing']['ga4_event_name'] = $quick_order_event_name;
				$json['remarketing']['ga4_datalayer']['event'] = $quick_order_event_name; 
			} else {
				$json['remarketing']['ga4_event_name'] = 'purchase';
			}			
			$json['remarketing']['ga4_quick_order_event_name'] = $order_info['ga4_event']; 
			...
			$json['remarketing']['ads_conversion_event'] = [ 
				'send_to'        => $json['remarketing']['google_ads_quick_identifier'],
				'value'          => $order_info['google_total'],
	...
	public function trackUtm() {
		...
		if (empty($this->session->data['fbp'])) {
			$this->session->data['fbp'] = 'fb' . '.' . '1' . '.' . round(microtime(true) * 1000) . '.' . mt_rand(1000000000, 9999999999);
		}
		...
		if (empty($this->session->data['fbc'])) {
	...
	public function processCategory($category_info = [], $heading_title = '', $results = []) {
	  ...
	  if ($this->config->get('remarketing_tiktok_status')) { 
		...
		if ($this->config->get('remarketing_ecommerce_ga4_status') || $this->config->get('remarketing_ecommerce_ga4_measurement_status')) {
			...
			$data['ga4_currency'] = $this->config->get('remarketing_ecommerce_currency');
			$data['ga4_send_to'] = $this->config->get('remarketing_ecommerce_ga4_identifier');
	...
	public function processProduct($product_info = [], $category_info = []) {
		...
		if ($this->config->get('remarketing_ecommerce_status') || $this->config->get('remarketing_ecommerce_ga4_status') || $this->config->get('remarketing_ecommerce_ga4_measurement_status')) {
			...
			$ecommerce_ga4_product['item_list_name'] = (!empty($category_info['name']) ? $category_info['name'] : addslashes($ga4_categories[0]));
			if (!empty($ga4_categories[0])) $ecommerce_ga4_product['item_category']  = addslashes($ga4_categories[0]); 
			if (!empty($ga4_categories[1])) $ecommerce_ga4_product['item_category2'] = addslashes($ga4_categories[1]);
			if (!empty($ga4_categories[2])) $ecommerce_ga4_product['item_category3'] = addslashes($ga4_categories[2]);
			if (!empty($ga4_categories[3])) $ecommerce_ga4_product['item_category4'] = addslashes($ga4_categories[3]);
	...
	public function processOrder($order_id = 0, $order_status_id = 0, $order_info = []) {
		...
		if ($this->config->get('remarketing_ecommerce_ga4_measurement_status') || $this->config->get('remarketing_ga4_only_purchase')) {
			...
			if ($event_name) { 
				...
				if ($ecommerce_info['order_info']['shipping_method']) $params['shipping_tier'] = $ecommerce_info['order_info']['shipping_method'];
				if ($ecommerce_info['order_info']['payment_method'])  $params['payment_type'] = $ecommerce_info['order_info']['payment_method'];
				...
				$this->sendGa4($ecommerce_data, $ecommerce_info);
		...
		if ($this->config->get('remarketing_esputnik_status')) {
			...
			if ($event_type && $esputnik_status && !empty($ecommerce_info['email'])) {
				...
				$event->params[] = ['name' => 'email', 'value' => $ecommerce_info['email']];
				...
				if ($esputnik_status == 'INITIALIZED') {
					...
					$json_contact_value->contacts = [];  
					$json_contact_value->dedupeOn = 'email'; 
					$json_contact_value->eventKeyForNewContacts = 'sp_remarketing';  
					$contact->channels = [['type'=>'email', 'value' => $ecommerce_info['email']], ['type'=>'sms', 'value' => $ecommerce_info['telephone']]];
					$contact->firstName = $ecommerce_info['firstname'];
					$contact->lastName = $ecommerce_info['lastname']; 
					$contact->externalCustomerId = $ecommerce_info['email'];
					if ($ecommerce_info['order_info']['customer_id'] > 0) {
						$contact->id = $ecommerce_info['order_info']['customer_id'];
					}
					$json_contact_value->contacts[] = $contact;
	...
	public function getOrderRemarketing($order_id) {
        ...
        if ($order_query->num_rows) {
			...
			if ($this->config->get('remarketing_max_order_value')) {
				...
				if ((int)$max_value > 0 && (int)$order_query->row['total'] > (int)$max_value) {
					$order_query->row['total'] = (int)$max_value;
				}
			}
			...
			$ga4_event = [
				...
				'shipping'       => $this->currency->format($shipping, $ecommerce_currency, '', false),
				'shipping_tier'  => $order_info['shipping_method'],
				'payment_type'   => $order_info['payment_method'],
				'tax'            => $this->currency->format($tax, $ecommerce_currency, '', false),
				'items'          => $ga4_products
			];
			...
			if ($order_info['email']) $ga4_datalayer['user_mail'] = $order_info['email']; 
			if ($order_info['telephone']) $ga4_datalayer['user_phone'] = $this->phoneClear($order_info['telephone']);
			...
			$parameters = [
				'uuid', 'ga4_uuid', 'fbclid', 'fbc', 'fbp', 'gclid', 'dclid', 'utm_source', 'utm_campaign', 'utm_medium', 'utm_term', 'utm_content', 'utm_referrer', 'ttclid', 'first_referrer', 'last_referrer' 
			]; 
			...
			if ($this->config->get('remarketing_autoclear_mode')) {
				$this->db->query("DELETE FROM `" . DB_PREFIX . "remarketing_orders` WHERE `date_added` < DATE_SUB(NOW(), INTERVAL 6 MONTH)");	
			}
