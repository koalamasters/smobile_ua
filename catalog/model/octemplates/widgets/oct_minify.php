...
class ModelOCTemplatesWidgetsOctMinify extends Model {
	public function octMinifyCss($data) {
		$oct_showcase_data = isset($this->config->get('theme_oct_showcase_data')['minify']) ? 1 : 0;
		$oct_styles = $this->document->getOctStyles();

		if ($oct_showcase_data) {
			$oct_showcase_version = "?v=" . $this->config->get('theme_oct_showcase_version');
	...
	public function octMinifyJs() {
		...
		if ($oct_showcase_data) {
			...
			if (file_exists($dir_minify . 'cache/' . $oct_name)) {
				...
				//$oct_scripts[0] = 'min/cache/' . $oct_name . $oct_showcase_version;
