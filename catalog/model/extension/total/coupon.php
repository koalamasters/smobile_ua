...
class ModelExtensionTotalCoupon extends Model {
	public function getCoupon($code) {
		...
		if ($status) {
			return array(
				...
				'ignore_special'=> $coupon_query->row['ignore_special'],
				'admitad'       => $coupon_query->row['admitad'],
				...
				foreach ($this->cart->getProducts() as $product) {
                    ...
                    if($coupon_info['ignore_special'] == 1) {
                        ...
                        if ($product_info && (float)$product_info['special'] > 0) {
                            continue; // Пропускаємо товар, якщо вже має акційну ціну
                        }
                    }
    ...
    public function addCouponEsputnik($data) {
    ...
    public function getOrderCouponCode($order_id)
    {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_total WHERE order_id = " . (int)$order_id . " AND code = 'coupon'");
        if ($query->num_rows) {
            if (preg_match('/\((.*?)\)/', $query->row['title'], $matches)) {
                return $matches[1]; // Наприклад: admitad-t-2
            }
        }
        return null;
    }
