...
class ControllerExtensionThemeOCTShowCase extends Controller {
	private $error = [];
	private $gretret = [];
    ...
    private function generateCss($data, $css_code, $store_id) {
		...
		if (isset($data['main_color']) && !empty($data['main_color'])) {
			...
			$css .= ".sc-btn-outline-primary:not([disabled]), .header-buttons .sc-btn.header-buttons-catalog, .header-buttons .sc-btn:hover {color:" . $data['main_color'] . ";}" . PHP_EOL;
			$css .= ".header-buttons-catalog path {stroke:" . $data['main_color'] . ";}" . PHP_EOL;
		}
		...
		if (isset($data['button_color_main']) && !empty($data['button_color_main'])) {
			$css .= ".sc-btn-primary:not([disabled]), button[data-target=\"#ocfilter-hidden-options\"], #ocfilter .btn-primary, #ocfilter .btn-primary:active, #ocfilter .btn-primary:focus, .simplecheckout-cart-buttons .button:hover, .simplecheckout-button-right .button:hover, #simplecheckout_button_login:hover, .pagination li.active span, .pagination li:hover a, #sc_livesearch_close, .simplecheckout-cart-buttons .button, .sc-header-location.active .sc-btn, .sc-btn-secondary:not([disabled]).clicked, .sc-btn-secondary:not([disabled]):hover, .sc-category-sort button.active, .sc-btn-outline-secondary:not(#oct-blog-search-button, #sc-search-button, [disabled]):hover, .header-search .sc-btn-with-icon:hover .sc-btn-icon, .sc-blog-search .sc-btn-with-icon:hover .sc-btn-icon, .sc-btn.sc-btn-secondary:not([disabled]):hover, .sc-switcher, .sidebar-main-menu-item-catalog .sidebar-main-menu-item-icon, .sc-btn-outline-primary:not([disabled]):hover, .sc-location.active .sc-btn-secondary {background-color:". $data['button_color_main'] .";}".PHP_EOL;
			$css .= ".sc-btn-outline-primary:not([disabled]) {border-color:" . $data['button_color_main'] . ";}" . PHP_EOL;
			$css .= ".sc-btn-outline-primary:not([disabled]) {color:" . $data['button_color_main'] . ";}" . PHP_EOL;
		}

		if (isset($data['button_color_text']) && !empty($data['button_color_text'])) {
			$css .= ".sc-btn.primary, .sc-btn-primary:not([disabled]), .sc-btn.primary .sc-btn-text, .sc-btn-primary:not([disabled]).sc-btn-text, .sc-btn.sc-btn-secondary:not([disabled]):hover .sc-btn-text, .sc-btn.sc-btn-secondary:not([disabled]).clicked .sc-btn-text, .sc-btn-outline-primary:not([disabled]):hover {color:". $data['button_color_text'] .";}".PHP_EOL;
			$css .= ".sc-btn-primary:not([disabled]) .sc-btn-icon {background:" . $data['button_color_text'] . ";}" . PHP_EOL;
			$css .= ".sc-btn.sc-btn-secondary:not([disabled]):hover .sc-btn-icon {background:" . $data['button_color_text'] . ";}" . PHP_EOL;
		}

		if (isset($data['button_fon_light']) && !empty($data['button_fon_light'])) {
			$css .= ".sc-btn-secondary:not([disabled]) {background-color:". $data['button_fon_light'] .";}".PHP_EOL;
		}

		if (isset($data['button_text_light']) && !empty($data['button_text_light'])) {
			$css .= ".sc-btn-secondary, .sc-btn-secondary .sc-btn-text {color:". $data['button_text_light'] .";}".PHP_EOL;
			$css .= ".sc-btn-secondary .sc-btn-icon {background:" . $data['button_text_light'] . ";}" . PHP_EOL;
		}
		...
		if (isset($data['top_link_color']) && !empty($data['top_link_color'])) {
			$css .= ".top-left li a, #top .sc-dropdown-toggle, .top-right .popup-call-trigger {color:". $data['top_link_color'] .";}".PHP_EOL;
		}

		if (isset($data['top_link_color_hover']) && !empty($data['top_link_color_hover'])) {
			$css .= ".top-left li a:hover, #top .sc-dropdown-toggle:hover, .top-right .popup-call-trigger:hover {color:". $data['top_link_color_hover'] .";}".PHP_EOL;
		}

		if (isset($data['footer_fon_color']) && !empty($data['footer_fon_color'])) {
			$css .= ".sc-footer {background-color:". $data['footer_fon_color'] .";}".PHP_EOL;
		}

		if (isset($data['footer_text_color']) && !empty($data['footer_text_color'])) {
			$css .= ".sc-footer, .sc-footer-subscribe .form-check-label, .sc-footer-credits {color:". $data['footer_text_color'] .";}".PHP_EOL;
		}

		if (isset($data['footer_link_color']) && !empty($data['footer_link_color'])) {
			$css .= ".sc-footer-phones, .sc-footer-list a, .sc-footer-email {color:". $data['footer_link_color'] .";}".PHP_EOL;
		}

		if (isset($data['footer_link_hover_color']) && !empty($data['footer_link_hover_color'])) {
			$css .= ".sc-footer-phones a:hover, .sc-footer-list a:hover, .sc-footer-email:hover {color:". $data['footer_link_hover_color'] .";}".PHP_EOL;
		}

		if (isset($data['category_module_fon_color']) && !empty($data['category_module_fon_color'])) {
			$css .= ".sc-column-categories-item {background:". $data['category_module_fon_color'] .";}".PHP_EOL;
		}

		if (isset($data['category_module_link_color']) && !empty($data['category_module_link_color'])) {
			$css .= ".sc-column-categories-item {color:". $data['category_module_link_color'] .";}".PHP_EOL;
		}

		if (isset($data['category_module_link_hover_color']) && !empty($data['category_module_link_hover_color'])) {
			$css .= ".sc-column-categories-item.active > span, .sc-column-categories-item .sc-categories-3 .active, .sc-column-categories-item a:hover, .sc-column-categories .sc-categories-3 li.active a {color:". $data['category_module_link_hover_color'] .";}".PHP_EOL;
		}
		...
		if (isset($data['modal_fon_icon_color']) && !empty($data['modal_fon_icon_color'])) {
			$css .= ".modal-header .btn-close {color:". $data['modal_fon_icon_color'] .";}".PHP_EOL;
		}

		if (isset($data['mobile_fon_top_color']) && !empty($data['mobile_fon_top_color'])) {
			$css .= "@media (max-width: 767px) {.header {background:". $data['mobile_fon_top_color'] .";}}".PHP_EOL;
		}

		if (!isset($data['compare'])) {
			$css .= ".header-buttons-compare, .sc-compare-btn {display: none!important;}".PHP_EOL;
		}

		if (!isset($data['wishlist'])) {
			$css .= ".header-buttons-wishlist, .sc-wishlist-btn {display: none!important;}".PHP_EOL;
		}
	...
	public function octUninstallEvents() {
		...
		$eventCodes = [
			'octemplates-showcase-header',
			'octemplates-showcase-footer',
			'octemplates-showcase-google-ecommerce-set-data',
			'octemplates-showcase-google-ecommerce-get-data',
			'octemplates-showcase-live-search',
			'octemplates-showcase-microdata',
			'octemplates-showcase-admin-user-account-frontend',
			'octemplates-showcase-user-account-frontend',
			'octemplates-showcase-user-account-pageinfo',
			'octemplates-showcase-admin-menu',
			'octemplates-showcase-product-views-history',
			'octemplates-showcase-admin-header',
			'octemplates-showcase-admin-header-front',
			'octemplates-showcase-admin-header-front-replace',
			'octemplates-showcase-category-module',
			'octemplates-showcase-menu-module',
			'octemplates-showcase-catalog-product-view-before-modification'
		];
	...
	public function update() {
		...
		$filesToDelete = [
			'oct_sc_fastorder.ocmod.xml',
			'oct_analytics.ocmod.xml',
			'oct_404_page.ocmod.xml',
			'oct_live_search.ocmod.xml',
			'oct_showcase_breadcrumbs.ocmod.xml',
			'oct_microdata.ocmod.xml',
			'oct_account_module.ocmod.xml',
			'oct_showcase_admin_part.ocmod.xml',
			'oct_product_views.ocmod.xml',
			'oct_megamenu.ocmod.xml',
			'oct_popup_purchase.ocmod.xml',
			'oct_showcase_minify.ocmod.xml',
			'oct_popup_view.ocmod.xml',
			'oct_atributes_view.ocmod.xml',
			'oct_showcase_webp.ocmod.xml',
			'oct_disable_buy_no_stock.ocmod.xml',
			'oct_cool_email_template.ocmod.xml'
		];
	...
	public function install() {
		...
		$this->model_setting_setting->editSetting('theme_'. $this->oct_theme, [
			...
			'theme_'. $this->oct_theme .'_image_logo_width' => 120,
			'theme_'. $this->oct_theme .'_image_logo_height' => 40,
			...
			'theme_'. $this->oct_theme .'_image_thumb_width' => 600,
			'theme_'. $this->oct_theme .'_image_thumb_height' => 600,
			'theme_'. $this->oct_theme .'_image_popup_width' => 600,
			'theme_'. $this->oct_theme .'_image_popup_height' => 600,
			'theme_'. $this->oct_theme .'_image_product_width' => 228,
			'theme_'. $this->oct_theme .'_image_product_height' => 228,
			'theme_'. $this->oct_theme .'_image_additional_width' => 40,
			'theme_'. $this->oct_theme .'_image_additional_height' => 60,
			...
			'theme_'. $this->oct_theme .'_lazyload_image' => 'catalog/showcase/lazy-image.svg',
			'theme_'. $this->oct_theme .'_css_code' => '',
			'theme_'. $this->oct_theme .'_js_code' => '',
			...
			'theme_'. $this->oct_theme .'_data_colors' => [
				'main_color' => 'rgb(79, 67, 227)',
				'main_link_color' => 'rgb(0, 99, 247)',
				'fon_color' => 'rgb(248, 249, 250)',
				'button_color_main' => 'rgb(79, 67, 227)',
				'button_color_text' => 'rgb(255, 255, 255)',
				'button_fon_light' => 'rgb(242, 244, 248)',
				'button_text_light' => 'rgb(28, 28, 40)',
				'top_fon_color' => 'rgb(40, 33, 130)',
				'top_link_color' => 'rgb(255, 255, 255)',
				'top_link_color_hover' => 'rgb(255, 255, 255)',
				'footer_fon_color' => 'rgb(42, 37, 104)',
				'footer_text_color' => 'rgb(199, 201, 217)',
				'footer_link_color' => 'rgb(199, 201, 217)',
				'footer_link_hover_color' => 'rgb(255, 255, 255)',
				...
				'mobile_fon_top_color' => 'rgb(255, 255, 255)',
				'modal_fon_title_color' => 'rgb(242, 244, 248)',
				'modal_text_title_color' => 'rgb(28, 28, 40)',
	...
	public function octQuickStart() {
		if (file_exists(DIR_SYSTEM ."library/octemplates/quickstart/". $this->oct_theme .".sql") && $this->config->get('theme_'. $this->oct_theme .'_license')) {
			...
			if ($lines) {
				...
				$this->db->query("INSERT INTO `" . DB_PREFIX . "setting` SET `store_id` = '0', `code` = 'theme_oct_showcase', `key` = 'theme_". $this->oct_theme ."_license', `value` = '". $this->db->escape($oct_license) ."'");
