			...
			if (!is_array($columns)) {
				...
				foreach ($columns as $column => $attr) {
					if ($column == 'store') {
						...
						if ((int)$result['status'] || !$this->config->get('module_admin_quick_edit_highlight_status')) {
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				if ($this->model_extension_module_aqe_catalog_information->quickEditInformation($_id, $column, $value, $lang_id, $this->request->post)) {
					$results['done'][] = $_id;
				} else {
					$results['failed'][] = $_id;
				}
