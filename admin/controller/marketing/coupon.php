...
// 🔧 Перевірка допустимих полів сортування
        $allowed_sort_fields = ['name', 'code', 'discount', 'date_start', 'date_end', 'status', 'purpose'];
        ...
        foreach ($results as $result) {
            $data['coupons'][] = array(
                ...
                'purpose'    => $result['purpose'], // 👈 додано нове поле
        ...
        $data['sort_purpose'] = $this->url->link('marketing/coupon', 'user_token=' . $this->session->data['user_token'] . '&sort=purpose' . $url, true); // 👈 додано посилання сортування
	...
	protected function getForm() {
        ...
        if (isset($this->request->post['ignore_special'])) {
            $data['ignore_special'] = $this->request->post['ignore_special'];
        } elseif (!empty($coupon_info)) {
            $data['ignore_special'] = $coupon_info['ignore_special'];
        } else {
            $data['ignore_special'] = 0;
        }

        if (isset($this->request->post['admitad'])) {
            $data['admitad'] = $this->request->post['admitad'];
        } elseif (!empty($coupon_info)) {
            $data['admitad'] = $coupon_info['admitad'];
        } else {
            $data['admitad'] = 0;
        }
        ...
        if (isset($this->request->post['purpose'])) {
            ...
            $data['purpose'] = $coupon_info['purpose'];
        } else {
            $data['purpose'] = '';
        }
