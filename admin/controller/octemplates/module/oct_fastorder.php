...
class ControllerOCTemplatesModuleOctFastorder extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (isset($this->request->post['oct_fastorder_data']['status']) && $this->request->post['oct_fastorder_data']['status'] == 1) {
				$this->addEvent();
			} else {
				$this->deleteEvent();
			}
  ...
  public function country() {
	...
	public function addEvent() {
		...
		if ($this->model_setting_event->getEventByCode('octemplates-showcase-fastorder')) {
			return;
		}
		...
		$events = [
			[
				'code' => 'octemplates-showcase-fastorder',
				'trigger' => 'catalog/controller/checkout/checkout/before',
				'action' => 'octemplates/events/fastorder'
			]
		];
	...
	public function deleteEvent() {
		...
		$eventCodes = [
			'octemplates-showcase-fastorder'
		];
