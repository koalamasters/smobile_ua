...
class ControllerOCTemplatesEventsAdminMenu extends Controller {
    ...
    public function index(&$route, &$data) {
        ...
        if ($this->user->hasPermission('access', 'extension/theme/oct_showcase')) {
            $oct_showcase[] = array(
                'name'	   => $this->language->get('text_oct_showcase'),
                'href'     => $this->url->link('extension/theme/oct_showcase', 'user_token=' . $this->session->data['user_token']. '&store_id=0', true),
                'children' => []
            );
        }
