...
class ControllerOCTemplatesEventsAdminHeader extends Controller {
    ...
    public function headerBack(&$route, &$data) {
        ...
        $this->language->load('octemplates/oct_showcase');

        if ($this->config->get('theme_oct_showcase_alert_status') && isset($this->session->data['user_token'])) {
            $data['oct_alert_status'] = $this->config->get('theme_oct_showcase_alert_status');

            $data['oct_alert_data'] = $oct_alert_data = $this->config->get('theme_oct_showcase_alert_data');
            $oct_total_calls = $oct_total_found_cheaper = $oct_total_reviews = $oct_total_stock_notifier = $oct_total_faqs = $product_total = $review_total = $return_total = 0;
