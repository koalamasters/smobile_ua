<?php 
class ModelExtensionFlowytrackingTools extends Model {

    public function __construct($registry) {
        parent::__construct($registry);

        $loader = new Loader($registry);
        $loader->language('flowytracking/tools');
    ...
    //Anothers functions
        ...
        public function getOptionValuesSelect($empty, $option_id)
        {
            $this->load->model('catalog/option');
        ...
        public function getGeoZones($empty = true) {
            $this->load->model('localisation/geo_zone');

            $countries_select = array();

            if($empty)
                $countries_select = array('' => $this->language->get('text_none_geo_zone'));

            $geo_zones = $this->model_localisation_geo_zone->getGeoZones();
            $geo_zones = $this->aasort($geo_zones, 'name');

            foreach ($geo_zones as $key => $atgr) {
                $countries_select[$atgr['geo_zone_id']] = $atgr['name'];
            }
    ...
    //Devman Extensions - info@flowytracking.com - 2017-10-07 09:53:22 - Functions related with get form and values
         public function ajax_get_form()
        {
            ...
            $result = $this->curl_call($data, $this->api_url.'resources/opencart/get_form.php');
                ...
                if($tab_has_fields)
                {
                    foreach ($tab['fields'] as $key_field => $field) {
                        ...
                        elseif($field['type'] == 'table_inputs')
                        {
                            ...
                            //Devman Extensions - info@flowytracking.com - 2017-09-04 20:23:33 - Search image fields inside table inputs
                            ...
                            if($is_multistore)
                            {
                                ...
                                foreach ($this->stores as $store_key => $store) {
                                    ...
                                    //Devman Extensions - info@flowytracking.com - 2017-09-04 20:25:12 - If table input has come image field, generate thumbs
                                    ...
                                    if($is_multistore)
                                    {
                                        ...
                                        foreach ($this->stores as $store_key => $store) {
                                            ...
                                            foreach ($this->langs as $key => $lang) {
                                                ...
                                                //Devman Extensions - info@flowytracking.com - 2017-09-04 20:00:08 - Add thumb if is a image field
                                                if($field['type'] == 'image')
                                                {
                                                    $thumb = !empty($value) ? $this->model_tool_image->resize($value, 100, 100) : $this->no_image_thumb;
	...
	public function getPaymentMethods()
    {
        ...
        $path_extensions = version_compare(VERSION, '2.2.0.0', '<=') ? 'controller/payment/*.php' : 'controller/extension/payment/*.php' ;
        $path_language = version_compare(VERSION, '2.2.0.0', '<=') ? 'payment/' : 'extension/payment/' ;

        foreach ($extensions as $ext) {
            array_push($codes,$ext);
        }

        $files = glob(DIR_APPLICATION . $path_extensions  );

        if ($files) {
            foreach ($files as $file) {
                $extension = basename($file, '.php');
                if (in_array($extension, $extensions)) {
                    $this->load->language($path_language . $extension, 'extension');

                    array_push($names,$this->oc2 ||version_compare(VERSION, '2.2.0.0', '<=') ? $this->language->get('heading_title') : $this->language->get('extension')->get('heading_title'));
                }
            }
        }
