...
class ControllerCommonFileManager extends Controller {
	public function index() {
               ...
               if (isset($this->request->get['filetype'])) {
		...
		if (substr(str_replace('\\', '/', realpath($directory) . '/' . $filter_name), 0, strlen(DIR_IMAGE . 'catalog')) == str_replace('\\', '/', DIR_IMAGE . 'catalog')) {
			...
			// Get files
			$files = glob($directory . '/' . $filter_name . '*.{jpg,jpeg,png,gif,svg,SVG,JPG,JPEG,PNG,GIF,avif,AVIF}', GLOB_BRACE);
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
               ...
               if (isset($url)) {
                ...
                if ($ext == 'avif') {
                    $thumb = HTTP_CATALOG . 'image/' . utf8_substr($image, utf8_strlen(DIR_IMAGE));
                } else {
		...
		$data['refresh'] = $this->url->link('common/filemanager', 'user_token=' . $this->session->data['user_token'] . $url, true);
               ...
               $data['refresh'] .= "&filetype=" . $filetype;
		...
		$pagination->limit = 16;
               ...
               $pagination->url = $this->url->link('common/filemanager', 'user_token=' . $this->session->data['user_token'] . $url .  "&filetype=" . $filetype . '&page={page}', true);
...
//						'image/mp4',
				...
				if ($this->config->get('module_tinify_status')) {

					if (in_array($file['type'], array('image/jpeg','image/pjpeg','image/png','image/x-png'))) {

						require_once(DIR_SYSTEM . "library/tinify/autoload.php");
				
						\Tinify\setKey($this->config->get('module_tinify_apikey'));

						$current_image = $directory . '/' . $filename;

						$source = \Tinify\fromFile($current_image);

						$source->toFile($current_image);
					}
