...
class ControllerCatalogCategory extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if(isset($this->request->post['attributeCategory'])) {
                $this->model_catalog_category->setAttributeCategory($category_id, $this->request->post['attributeCategory']);
            }
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if (isset($this->request->post['attributeCategory'])) {
                $this->model_catalog_category->setAttributeCategory($this->request->get['category_id'], $this->request->post['attributeCategory']);
            }else{
                $this->model_catalog_category->setAttributeCategory($this->request->get['category_id'], 0);
            }

            /**
             * 09/04/2024
             */
            if (isset($this->request->post['cat_products_old']) && isset($this->request->post['cat_products_new'])) {

                $productsSkuByCat = $this->model_catalog_category->getProductsSkuByCat($this->request->get['category_id']);
                $cat_products_old = array_filter(explode(',',$this->request->post['cat_products_old']));
                $cat_products_new = array_filter(explode(',',$this->request->post['cat_products_new']));

                // Шукаємо товари для видалення з категроії
                $products_for_delete = [];
                foreach ($cat_products_old as $old_prod){
                    if(!in_array($old_prod, $cat_products_new)){

                        foreach ($productsSkuByCat as $product){
                            if($product['sku'] == $old_prod){
                                $products_for_delete[] = $product['product_id'];
                            }
                        }

                    }
                }
                if(!empty($products_for_delete)) {
                    $this->model_catalog_category->deleteProductFromCategory($products_for_delete, $this->request->get['category_id']);
                }

                // Шукаємо товари для додавання з категроії
                $products_for_add_sku = [];
                foreach ($cat_products_new as $new_prod){
                    if(!in_array($new_prod, $cat_products_old)){
                        $product_id = $this->model_catalog_category->getProductIdBySKU($new_prod);
                        $this->model_catalog_category->addProductToCategory($product_id, $this->request->get['category_id']);
                    }
                }
            }
	...
	protected function getForm() {
			...
			$data['oct_showcase_seo_title_data'] = $this->config->get('theme_oct_showcase_seo_title_data');
    ...
    // SEO URL Generator FREE . begin
		...
		$data['languages'] = $this->model_localisation_language->getLanguages();
			...
			if (isset($oct_showcase_data['category_page']) && $oct_showcase_data['category_page']) {
				$data['oct_cat_page'] = $oct_showcase_data['category_page'];
    ...
    if (isset($this->request->post['code'])) {
        ...
        $data['attributeCategories'] = $this->model_catalog_category->getAttributeCategories();
        if(isset($this->request->get['category_id'])) {
            $data['CurentAttributeCategory'] = $this->model_catalog_category->getAttributeCategory($this->request->get['category_id']);
        }
         ...
         */
        if(isset($this->request->get['category_id'])) {
            $product_list = $this->model_catalog_category->getProductsSkuByCat($this->request->get['category_id']);
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['category_description'] as $language_id => $value) {
			...
			if (((utf8_strlen($value['meta_title']) < 1) || (utf8_strlen($value['meta_title']) > 255)) && !isset($oct_showcase_seo_title_data['category']['title_empty'])) {
