...
class ControllerCatalogReview extends Controller {
            ...
            if (isset($this->request->files['review_photos']) && is_array($this->request->files['review_photos']['name'])) {
                foreach ($this->request->files['review_photos']['name'] as $key => $value) {
                    if (is_uploaded_file($this->request->files['review_photos']['tmp_name'][$key])) {
                        // Отримуємо початкове ім'я файлу та приводимо його до нижнього регістру
                        $filename = strtolower(html_entity_decode($this->request->files['review_photos']['name'][$key], ENT_QUOTES, 'UTF-8'));

                        // Отримуємо розширення файлу
                        $extension = pathinfo($filename, PATHINFO_EXTENSION);

                        // Генеруємо унікальне ім'я файлу з використанням md5 і timestamp
                        $new_filename = md5(uniqid(rand(), true)) . '_' . time() . '.' . $extension;

                        // Зберігаємо файл у визначену директорію (наприклад, 'image/catalog/review/')
                        move_uploaded_file($this->request->files['review_photos']['tmp_name'][$key], DIR_IMAGE . 'catalog/review/' . $new_filename);

                        // Додаємо запис про файл у вашу нову таблицю oc_review_attachment
                        $this->model_catalog_review->set_review_attachment($this->request->get['review_id'], 'image', 'catalog/review/' . $new_filename);
                    }
                }
            }
	...
	protected function getForm() {
		...
		if (isset($this->request->get['review_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            ...
            $review_atach = $this->model_catalog_review->get_review_attachments($this->request->get['review_id']);
            foreach ($review_atach as $attach){
                if($attach['type'] == 'video'){
                    $data['video'] = $attach['link'];
                }

                if($attach['type'] == 'image'){
                    $data['images'][$attach['attachment_id']] = $attach['link'];
                }
            }
		}
	...
	protected function validateForm() {
        ...
        if(isset($this->request->post['video']) && strlen($this->request->post['video']) > 0) {
            if (!$this->isYoutubeLink($this->request->post['video'])) {
                $this->error['video'] = 'Хибний формат посилання';
            }
        }
    ...
    public function deleteImage(){
        ...
        if(isset($this->request->get['attachment_id'])) {
            ...
            $this->model_catalog_review->delete_review_attachment($this->request->get['attachment_id']);
