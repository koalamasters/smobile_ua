...
class ControllerCatalogProduct extends Controller {
    ...
    public function add() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if (isset($this->request->post['selected_attribute_template'])) {
                $this->model_catalog_product->setProductAtrributeTemplate($product_id, $this->request->post['selected_attribute_template']);
            }

            if (isset($this->request->post['is_new']) && isset($this->request->post['new_end_date'])) {
                $this->model_catalog_product->setIsNew($product_id, $this->request->post['is_new'], $this->request->post['new_end_date']);
            }
    ...
    public function edit() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
$this->load->controller('extension/module/hpmrr/save');


            $this->model_catalog_product->editProduct($this->request->get['product_id'], $this->request->post);

            if(isset($this->request->post['selected_attribute_template'])) {
                $this->model_catalog_product->setProductAtrributeTemplate($this->request->get['product_id'], $this->request->post['selected_attribute_template']);
            }


            $this->model_catalog_product->setIsNew($this->request->get['product_id'], $this->request->post['is_new'], $this->request->post['new_end_date']);





				if ($this->config->get('module_preorder_edit_product_alert')) {
    ...
    public function delete() {
        ...
        if (isset($this->request->post['selected']) && $this->validateDelete()) {
            ...
            foreach ($this->request->post['selected'] as $product_id) {
                $this->model_catalog_product->deleteNewTag($product_id);
     ...
     'filter_import_batch'	  => $filter_import_batch,
        ...
        foreach ($results as $result) {
            ...
            $data['products'][] = array(
                ...
                'image'      => $image,
                'name'       => $result['name'],
'kj_parent'       => $this->db->query("SELECT `parent_id` FROM `" . DB_PREFIX . "hpmrr_links` WHERE `product_id` = '" . (int)$result['product_id'] . "' OR `parent_id` = '" . (int)$result['product_id'] . "'")->num_rows,
                'model'      => $result['model'],
    ...
    protected function getForm($from = '') {
    ...
    if (isset($this->request->get['filter_import_batch'])) {
      $url .= '&filter_import_batch=' . urlencode(html_entity_decode($this->request->get['filter_import_batch'], ENT_QUOTES, 'UTF-8'));
		...
		if ($this->config->get($type.'ukrcredits_status')) {
			...
			$data['credit_type_sb'] = $this->language->get('credit_type_sb');
			$data['credit_type_aa'] = $this->language->get('credit_type_aa');
			$data['credit_type_pl'] = $this->language->get('credit_type_pl');
			$data['credit_type_pu'] = $this->language->get('credit_type_pu');
			$data['partsCount'] = $this->language->get('partsCount');
			$data['markup'] = $this->language->get('markup');
			$data['special'] = $this->language->get('special');
			...
			foreach ($banks as $bank) {
				if (isset($this->request->post['product_'.$bank])) {
					$data['product_pp'] = $this->request->post['product_'.$bank];
				} elseif (!empty($credit_info)) {
					$data['product_'.$bank] = $credit_info['product_'.$bank];
				} else {
					$data['product_'.$bank] = '';
				}

				if (isset($this->request->post['partscount_'.$bank])) {
					$data['partscount_pp'] = $this->request->post['partscount_'.$bank];
				} elseif (!empty($credit_info)) {
					$data['partscount_'.$bank] = $credit_info['partscount_'.$bank];
				} else {
					$data['partscount_'.$bank] = '';
				}

				if (isset($this->request->post['markup_'.$bank])) {
					$data['markup_pp'] = $this->request->post['markup_'.$bank];
				} elseif (!empty($credit_info)) {
					$data['markup_'.$bank] = $credit_info['markup_'.$bank];
				} else {
					$data['markup_'.$bank] = '';
				}

				if (isset($this->request->post['special_'.$bank])) {
					$data['special_pp'] = $this->request->post['special_'.$bank];
				} elseif (!empty($credit_info)) {
					$data['special_'.$bank] = $credit_info['special_'.$bank];
				} else {
					$data['special_'.$bank] = '';
				}
			}
...
$data['tab_hpm'] = $this->language->get('tab_hpm');
        $data['hpm'] = $this->load->controller('extension/module/hpmrr/getList');
        ...
        foreach ($product_attributes as $product_attribute) {
            ...
            $attr_values_html = $this->getAttributeValuesHTML($product_attribute['attribute_id']);
            $attr_type = $this->model_catalog_attribute->getAttAttributeType($product_attribute['attribute_id']);
            $attr_values = $this->model_catalog_attribute->getAttAttributeValues($product_attribute['attribute_id']);
            $attr_values_arr = $this->getAttrvibuteValuesArray($attr_values);
            ...
            if ($attribute_info) {
                $data['product_attributes'][] = array(
                    ...
                    'attribute_type' => ($attr_type) ? $attr_type : 'default',
                    'attribute_values' => $attr_values_arr,
                    'attr_values_html' => $attr_values_html
                );
            }
        }
        ...
        $categoryTemplates = $this->model_catalog_category->getCategoryTemplates();
        $categoryTemplatesList = $this->model_catalog_category->getAttributeTemlates();

        $categoryTemplatesListFull = [];
        foreach ($categoryTemplatesList as $catTemplate){
            $categoryTemplatesListFull[$catTemplate['attr_cat_id']] = $catTemplate['name'];
        }


        $cat_to_attr_template = [];
        foreach ($categoryTemplates as $categoryTemplate){
            $cat_to_attr_template[$categoryTemplate['category_id']] = $categoryTemplate['attribute_template_id'];
            $cat_to_attr_template_full[$categoryTemplate['category_id']] = $categoryTemplate;
        }

        $product_categories_id= [];
        $product_attributes_template = [];
        foreach ($data['product_categories'] as $cat) {
            $product_categories_id[] = $cat['category_id'];
            if(isset($cat_to_attr_template[$cat['category_id']]) && isset($categoryTemplatesListFull[$cat_to_attr_template[$cat['category_id']]])){
                $data['product_attributes_template'][$cat_to_attr_template[$cat['category_id']]] = $categoryTemplatesListFull[$cat_to_attr_template[$cat['category_id']]];
            }
        }
        ...
        /**
         * Встановлюэмо позначку Новинка
         */

        if($this->request->get['product_id']) {
            $data['is_new'] = $this->model_catalog_product->getIsNew($this->request->get['product_id']);



            if(empty($data['is_new']) && $days_passed < 31){
                $data['is_new']['is_new'] = 1;
            }
        }

        $data['future_30d'] = $future_date = date('Y-m-d', strtotime('+30 days'));
        ...
        if($this->request->get['product_id']) {
            $data['attr_tempalte_id'] = $this->model_catalog_product->getProductAttributeTemplate($this->request->get['product_id']);
        }else{
            $data['attr_tempalte_id'] = 0;
        }

        $data['category_templates'] = json_encode($cat_to_attr_template);
    ...
    public function getAttrvibuteValuesArray($attribute_values){
        ...
        return $attribute_values_arr;
    }

    public function getAttributeValuesHTML($attribute_id = 0){
        $response = '';
        ...
        $this->load->model('catalog/attribute');
        $attr_type = $this->model_catalog_attribute->getAttAttributeType($attribute_id);
        $attr_values = $this->model_catalog_attribute->getAttAttributeValues($attribute_id);
        $attr_values_arr = $this->getAttrvibuteValuesArray($attr_values);

        ob_start();?>
        <div class="attribute_values tmp_hidden" >
            <?if($attr_type == 'dropdown'){?>

                <select class="form-control attribute-row-values form-control" name="" id="" data-type="<?=$attr_type?>">
                    <option value="">Оберіть значення</option>
                    <?foreach ($attr_values_arr as $value){
                        $value['ua'] = str_replace('\n', '', $value['ua']);
                        $value['ua'] = str_replace("\n", '', $value['ua']);
                        $value['ru'] = str_replace('\n', '', $value['ru']);
                        $value['ru'] = str_replace("\n", '', $value['ru']);
                        $string = implode('|', $value);
                        $string = trim(preg_replace('/\s\s+/', ' ', $string));
                        ?>
                        <option value="<?=$string?>"><?=$value['ua']?></option>
                    <?}?>
                </select>
            <?}?>
            ...
            <?if($attr_type == 'checkbox'){?>
                ...
                <div class="attribute-row-values form-control" data-type="<?=$attr_type?>">
                    <?foreach ($attr_values_arr as $key => $value){
                        if(!$value['ua'])
                            continue;
                        ?>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" data-balue-ua="<?=$value['ua']?>" value="<?=trim(implode('|', $value))?>" id="attr-<?=$attribute_id?>-<?=$key?>" name="attr-<?=$attribute_id?>"  />
                            <label class="form-check-label" for="attr-<?=$attribute_id?>-<?=$key?>"><?=$value['ua']?></label>
                        </div>
    ...
    protected function validateForm() {
        ...
        foreach ($this->request->post['product_description'] as $language_id => $value) {
			...
			$oct_showcase_seo_title_data = $this->config->get('theme_oct_showcase_seo_title_data');

			if (((utf8_strlen($value['meta_title']) < 1) || (utf8_strlen($value['meta_title']) > 255)) && !isset($oct_showcase_seo_title_data['product']['title_empty'])) {
