...
class ControllerCatalogAttribute extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$attr_id = $this->model_catalog_attribute->addAttribute($this->request->post);

            $this->model_catalog_attribute->setAttributeType($attr_id, $this->request->post['input_type']);

            $esacped_string = $this->db->escape($this->request->post['input_values']);
            $this->model_catalog_attribute->setAttributeValues($attr_id, $esacped_string);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
             ...
             */
            //if($this->request->get['attribute_id'] == 128){
                $this->model_catalog_attribute->setAttributeType($this->request->get['attribute_id'], $this->request->post['input_type']);

                $esacped_string = $this->db->escape($this->request->post['input_values']);
                $this->model_catalog_attribute->setAttributeValues($this->request->get['attribute_id'], $esacped_string);
//                die();
            //}
	...
	protected function getForm() {
        ...
        if(isset($attribute_info['attribute_id'])) {
            $data['attribute_type'] = $this->model_catalog_attribute->getAttAttributeType($attribute_info['attribute_id']);
            $data['attribute_values'] = $this->model_catalog_attribute->getAttAttributeValues($attribute_info['attribute_id']);
            $data['attribute_values_arr'] = $this->getAttrvibuteValuesArray($data['attribute_values']);
            if(empty($data['attribute_type'])){
                $data['attribute_type'] = 'default';
            }
    ...
    public function getAttrvibuteValuesArray($attribute_values){
		...
		public function getTemplate(){
			$this->load->controller('extension/module/attribute_category/getTemplate');
		}
		public function getTemplateAttr(){
			$this->load->controller('extension/module/attribute_category/getTemplateAttr');
		}
