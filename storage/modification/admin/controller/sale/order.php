	...
	public function delete() {
		...
		$url = '';
         ...
         if (isset($this->request->get['filter_sid'])) {
            $url .= '&filter_sid=' . $this->request->get['filter_sid'];
          }
...
'filter_sid'      => $filter_sid,
        ...
        if(isset($result) && isset($result['order_id'])) {
          $order_id  = $result['order_id'];
        }
        
			$data['orders'][] = array(
'salesagentname' => $this->model_extension_module_salesagent->getAgentName($order_id),
				'order_id'      => $result['order_id'],
		...
		$data['filter_order_id'] = $filter_order_id;
         ...
         $data['filter_sid'] = $filter_sid;
         $data['entry_filter_sid'] = "Sales Agent";
         $this->load->model('extension/module/salesagent');
	...
	public function getForm() {
		...
		if (!empty($order_info)) {
          ...
          $this->load->model('extension/module/salesagent');
          $data['salesagent_id'] = $this->model_extension_module_salesagent->getAgentId($data['order_id']);
          $data['salesagents'] = $this->model_extension_module_salesagent->getsalesagents();
        ...
        if($salesagent_id && !empty($this->config->get('salesagent_orderstatuspermission')) && in_array($data['order_status_id'],$this->config->get('salesagent_orderstatuspermission'))) {
              $data['api_token'] = '';
        }
	...
	public function info() {
		...
		if ($order_info) {
         ...
         if (isset($this->request->get['filter_sid'])) {
          ...
          $this->load->model('extension/module/salesagent');
          $data['salesagentname'] = $this->model_extension_module_salesagent->getAgentName($order_id);
          $this->load->language('extension/module/salesagent_links');
          $salesagent_id =  $data['issalesagent'] = $this->user->getSalesAgentId();
          if($salesagent_id) {
             $data['salesagent_addorderhistory'] = $this->config->get('salesagent_addorderhistory');
          } else {
            $data['salesagent_addorderhistory'] = 1;
          }
...
//                );
        ...
        if($salesagent_id && !empty($this->config->get('salesagent_orderstatuspermission')) && in_array($order_info['order_status_id'],$this->config->get('salesagent_orderstatuspermission'))) {
              $data['api_token'] = '';
        }
	...
	public function invoice() {
        ...
        $this->load->language('extension/module/salesagent_links');
        $data['text_salesagentname'] = $this->language->get('text_salesagentname');
