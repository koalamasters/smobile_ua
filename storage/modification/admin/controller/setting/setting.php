...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
		...
		// Header image
		if (isset($this->request->post['config_image_header'])) {
			$data['config_image_header'] = $this->request->post['config_image_header'];
		} else {
			$data['config_image_header'] = $this->config->get('config_image_header');
		}

		if (isset($this->request->post['config_image_header']) && is_file(DIR_IMAGE . $this->request->post['config_image_header'])) {
			$data['thumb_header'] = $this->model_tool_image->resize($this->request->post['config_image_header'], 100, 100);
		} elseif ($this->config->get('config_image_header') && is_file(DIR_IMAGE . $this->config->get('config_image_header'))) {
			$data['thumb_header'] = $this->model_tool_image->resize($this->config->get('config_image_header'), 100, 100);
		} else {
			$data['thumb_header'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}

		// Header mobile image
		if (isset($this->request->post['config_image_header_mobile'])) {
			$data['config_image_header_mobile'] = $this->request->post['config_image_header_mobile'];
		} else {
			$data['config_image_header_mobile'] = $this->config->get('config_image_header_mobile');
		}

		if (isset($this->request->post['config_image_header_mobile']) && is_file(DIR_IMAGE . $this->request->post['config_image_header_mobile'])) {
			$data['thumb_header_mobile'] = $this->model_tool_image->resize($this->request->post['config_image_header_mobile'], 100, 100);
		} elseif ($this->config->get('config_image_header_mobile') && is_file(DIR_IMAGE . $this->config->get('config_image_header_mobile'))) {
			$data['thumb_header_mobile'] = $this->model_tool_image->resize($this->config->get('config_image_header_mobile'), 100, 100);
		} else {
			$data['thumb_header_mobile'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}

		// Footer image
		if (isset($this->request->post['config_image_footer'])) {
			$data['config_image_footer'] = $this->request->post['config_image_footer'];
		} else {
			$data['config_image_footer'] = $this->config->get('config_image_footer');
		}

		if (isset($this->request->post['config_image_footer']) && is_file(DIR_IMAGE . $this->request->post['config_image_footer'])) {
			$data['thumb_footer'] = $this->model_tool_image->resize($this->request->post['config_image_footer'], 100, 100);
		} elseif ($this->config->get('config_image_footer') && is_file(DIR_IMAGE . $this->config->get('config_image_footer'))) {
			$data['thumb_footer'] = $this->model_tool_image->resize($this->config->get('config_image_footer'), 100, 100);
		} else {
			$data['thumb_footer'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		...
		$data['captcha_pages'][] = array(
			...
			$this->load->language('octemplates/theme/oct_showcase');
