...
class ModelSaleOrder extends Model {
	public function getOrder($order_id) {
        ...
        $this->load->model("extension/module/salesagent");
          $result = $this->model_extension_module_salesagent->getOrderCheck($order_id);
          if($result) {
            return array();
          }
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS order_status, o.shipping_code, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
         ...
         if($salesagent_id OR (isset($data['filter_sid']) && !empty($data['filter_sid']))) {
		...
		if (!empty($data['filter_order_status'])) {
            ...
            if($this->request->get['show_all'] != 1){
	...
	public function getTotalOrders($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order`";
         ...
         $salesagent_id =  $this->user->getSalesAgentId();
         if($salesagent_id OR (isset($data['filter_sid']) && !empty($data['filter_sid']))) {
          $sql .= " INNER JOIN `" . DB_PREFIX . "salesagent_order` so ON (".DB_PREFIX."order.order_id = so.order_id) ";
         } 
        ...
        $sql .= " AND ".DB_PREFIX."order.order_id = '" . (int)$data['filter_order_id'] . "'";
