...
class ModelExtensionReportSale extends Model {
	public function getTotalSales($data = array()) {
        ...
        if($salesagent_id) {
          ...
          $sql .= " INNER JOIN `" . DB_PREFIX . "salesagent_order` so ON (o.order_id = so.order_id) WHERE order_status_id > '0' AND  so.salesagent_id IN (".$salesagent_id.") ";
         } else {
	...
	public function getTotalOrdersByCountry() {
        ...
        $salesagent_id =  $this->user->getSalesAgentId();
        if($salesagent_id) {
            $query = $this->db->query("SELECT COUNT(*) AS total, SUM(o.total) AS amount, c.iso_code_2 FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "country` c ON (o.payment_country_id = c.country_id) INNER JOIN `" . DB_PREFIX . "salesagent_order` so ON (o.order_id = so.order_id) WHERE o.order_status_id > '0'  AND so.salesagent_id IN (".$salesagent_id.") GROUP BY o.payment_country_id");
        } else {
	...
	public function getTotalOrdersByDay() {
        ...
        $salesagent_id =  $this->user->getSalesAgentId();
        if($salesagent_id) {
          $query = $this->db->query("SELECT COUNT(*) AS total, HOUR(date_added) AS hour FROM `" . DB_PREFIX . "order` o INNER JOIN `" . DB_PREFIX . "salesagent_order` so ON (o.order_id = so.order_id) WHERE order_status_id IN(" . implode(",", $implode) . ")  AND so.salesagent_id IN (".$salesagent_id.") AND DATE(date_added) = DATE(NOW()) GROUP BY HOUR(date_added) ORDER BY date_added ASC");
        } else {
	...
	public function getTotalOrdersByWeek() {
        ...
        $salesagent_id =  $this->user->getSalesAgentId();
        if($salesagent_id) {
          $query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` o INNER JOIN `" . DB_PREFIX . "salesagent_order` so ON (o.order_id = so.order_id) WHERE order_status_id IN(" . implode(",", $implode) . ") AND so.salesagent_id IN (".$salesagent_id.") AND DATE(date_added) >= DATE('" . $this->db->escape(date('Y-m-d', $date_start)) . "') GROUP BY DAYNAME(date_added)");
        } else {
	...
	public function getTotalOrdersByMonth() {
        ...
        $salesagent_id =  $this->user->getSalesAgentId();
        if($salesagent_id) {
          $query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` o INNER JOIN `" . DB_PREFIX . "salesagent_order` so ON (o.order_id = so.order_id) WHERE order_status_id IN(" . implode(",", $implode) . ") AND so.salesagent_id IN (".$salesagent_id.") AND DATE(date_added) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' GROUP BY DATE(date_added)");
        } else {
	...
	public function getTotalOrdersByYear() {
        ...
        $salesagent_id =  $this->user->getSalesAgentId();
        if($salesagent_id) {
          $query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` o INNER JOIN `" . DB_PREFIX . "salesagent_order` so ON (o.order_id = so.order_id) WHERE order_status_id IN(" . implode(",", $implode) . ") AND so.salesagent_id IN (".$salesagent_id.") AND YEAR(date_added) = YEAR(NOW()) GROUP BY MONTH(date_added)");
        } else {
	...
	public function getOrders($data = array()) {
        ...
        $salesagent_id =  $this->user->getSalesAgentId();
        if($salesagent_id) {
          $sql .= " INNER JOIN `" . DB_PREFIX . "salesagent_order` so ON (o.order_id = so.order_id) ";
        }   
        ...
        if($salesagent_id) {
          $sql .= " AND so.salesagent_id IN (".$salesagent_id.") ";
        } 
	...
	public function getTotalOrders($data = array()) {
        ...
        $salesagent_id =  $this->user->getSalesAgentId();
        if($salesagent_id) {
          $sql .= " o INNER JOIN `" . DB_PREFIX . "salesagent_order` so ON (o.order_id = so.order_id) ";
        }   
	...
	public function getTaxes($data = array()) {
        ...
        $salesagent_id =  $this->user->getSalesAgentId();
        if($salesagent_id) {
          $sql = "SELECT MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end, ot.title, SUM(ot.value) AS total, COUNT(o.order_id) AS `orders` FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_total` ot ON (ot.order_id = o.order_id) INNER JOIN `" . DB_PREFIX . "salesagent_order` so ON (o.order_id = so.order_id) WHERE ot.code = 'tax'";
	...
	public function getShipping($data = array()) {
        ...
        $sql = "SELECT MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end, ot.title, SUM(ot.value) AS total, COUNT(o.order_id) AS `orders` FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_total` ot ON (o.order_id = ot.order_id) INNER JOIN `" . DB_PREFIX . "salesagent_order` so ON (o.order_id = so.order_id) WHERE ot.code = 'shipping'";
