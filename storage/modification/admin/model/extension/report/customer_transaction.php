...
class ModelExtensionReportCustomerTransaction extends Model {
	...
	public function getTotalTransactions($data = array()) {
        ...
        $salesagent_id =  $this->user->getSalesAgentId();
        if($salesagent_id) {
          $sql = "SELECT COUNT(DISTINCT ct.customer_id) AS total FROM `" . DB_PREFIX . "customer_transaction` ct LEFT JOIN `" . DB_PREFIX . "customer` c ON (ct.customer_id = c.customer_id) INNER JOIN `" . DB_PREFIX . "salesagent_customer` sc ON (sc.customerid = c.customer_id)";
        } else{
