...
class ControllerStartupStartup extends Controller {
	public function index() {
        ...
        // Збереження admitad_uid
        if (isset($this->request->get['admitad_uid'])) {
            setcookie('admitad_uid', $this->request->get['admitad_uid'], time() + 30 * 24 * 3600, '/');
        }

        // Очищення при gclid без utm_source
        if (isset($this->request->get['gclid']) && !isset($this->request->get['utm_source'])) {
            setcookie('admitad_uid', '', time() - 3600, '/');
            unset($_COOKIE['admitad_uid']);
        }
...
$this->url->addRewrite(new Simple\Rewrite($this->config, $this->session));
        ...
        if(file_exists(DIR_SYSTEM."library/cart/salesagent.php")) {
          $this->registry->set('salesagent', new Cart\Salesagent($this->registry));
        }
		...
		$this->registry->set('encryption', new Encryption($this->config->get('config_encryption')));
            ...
            // Flowy Tracking - Startup libraries
                $file = 'library/flowytracking/includes/startup_libraries.php';
        ...
        // Автоматичне застосування купона по scode
        if (isset($this->request->get['scode'])) {
            $coupon = $this->request->get['scode'];

            // Завантажуємо модель
            $this->load->model('extension/total/coupon');

            // Перевіряємо, чи купон дійсний
