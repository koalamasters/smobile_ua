...
class ControllerMailOrder extends Controller
{
	...
	public function add($order_info, $order_status_id, $comment, $notify)
	{
		...
		$data['text_firstname'] = $data['firstname'];
		$data['text_lastname'] = $data['lastname'];
		...
		$data['products'] = array();
			...
			$oct_showcase_contact_address = $oct_showcase_data['contact_address'];
			...
			$data['oct_showcase_contact_address'] = (isset($oct_showcase_contact_address[$lang_id]) && !empty($oct_showcase_contact_address[$lang_id])) ? html_entity_decode($oct_showcase_contact_address[$lang_id], ENT_QUOTES, 'UTF-8') : false;

			$data['oct_showcase_contact_telephone'] = (isset($oct_showcase_data['contact_telephone']) && !empty($oct_showcase_data['contact_telephone'])) ? array_values(array_filter(explode(PHP_EOL, $oct_showcase_data['contact_telephone']))) : false;

			$data['oct_showcase_contact_open'] = (isset($oct_showcase_data['contact_open'][$lang_id]) && !empty($oct_showcase_data['contact_open'][$lang_id])) ? array_values(array_filter(explode(PHP_EOL, $oct_showcase_data['contact_open'][$lang_id]))) : false;

			$data['oct_showcase_socials'] = (isset($oct_showcase_data['socials']) && !empty($oct_showcase_data['socials'])) ? $oct_showcase_data['socials'] : false;
	...
	public function alert(&$route, &$args)
	{
		...
		if ($order_info && !$order_info['order_status_id'] && $order_status_id && in_array('order', (array) $this->config->get('config_mail_alert'))) {
			...
			if ($order_info['payment_address_format']) {
				$formatq = $order_info['payment_address_format'];
			} else {
				$formatq = '{firstname} {lastname}' . "\n" . '{company}' . "\n" . '{address_1}' . "\n" . '{address_2}' . "\n" . '{city} {postcode}' . "\n" . '{zone}' . "\n" . '{country}';
			}
			...
			$data['payment_address'] = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($findq, $replaceq, $formatq))));
	
			if ($order_info['shipping_address_format']) {
				$formatq = $order_info['shipping_address_format'];
			} else {
				$formatq = '{firstname} {lastname}' . "\n" . '{company}' . "\n" . '{address_1}' . "\n" . '{address_2}' . "\n" . '{city} {postcode}' . "\n" . '{zone}' . "\n" . '{country}';
			}
