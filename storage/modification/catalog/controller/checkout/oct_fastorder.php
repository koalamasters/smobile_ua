...
class ControllerCheckoutOctFastorder extends Controller {
    public function index() {
        ...
        $data['text_auth_with_goole'] = $this->language->get('text_auth_with_goole');

        $data['text_shipping_address'] = $this->language->get('text_shipping_address');
        $data['additinal_wait_text'] = $this->language->get('additinal_wait');
        ...
        foreach ($products as $product) {
            ...
            $data['tmp_products'][] = $tmp_arr;
            $data['products'][] = $tmp_arr;
        ...
        $this->document->addStyle('catalog/view/theme/oct_showcase/stylesheet/oct-fastorder.min.css');
        ...
        $data['products'] = $data['tmp_products'];



//        if (!empty($data['totals'])) {
//            $last_total = $data['totals'][array_key_last($data['totals'])];
//            $last_total_price = str_replace('грн', '', $last_total['text']);
//            $last_total_price = str_replace(' ', '', $last_total_price);
//            $data['total_price_for_ads'] = (int)$last_total_price;
//        }
        ...
        $data['totals'][array_key_last($data['totals'])]['total_sum'] = true;
        ...
        $data['delivery_carrier_tariffs'] = $this->language->get('delivery_carrier_tariffs');
        $data['delivery_free'] = $this->language->get('delivery_free');
        $data['no_call_notice'] = $this->language->get('no_call_notice');
        $data['text_cart'] = $this->language->get('cart');
        $data['full_total'] = $this->language->get('full_total');
        $data['text_edit_cart'] = $this->language->get('edit_cart');
        $data['edit_cart_link'] = $this->url->link('checkout/cart');

        $data['cart_notice'] = $this->language->get('cart_notice');
    ...
    public function validate_form($data = array()) {
        ...
        if (isset($this->request->post['telephone'])) {
            ...
            // Перевіряємо, що номер починається з коду країни +380 та містить 12 цифр
            if (!preg_match('/^380\d{9}$/', $telephone)) {
        ...
        if (!$json) {
            ...
            if (isset($this->request->post['shipping_address']) && $this->request->post['shipping_address'] == 'new') {
                ...
                if (!$json) {
                    ...
                    //$this->session->data['shipping_country_id'] = (isset($this->request->post['shipping_country_id']) && !empty($this->request->post['shipping_country_id'])) ? $this->request->post['shipping_address_id'] : '';
    ...
    public function cart($render = true, &$data = array()) {
        ...
        $total_data = array(
            ...
            $totals[] = [
                'code' => 'special',
                'title' => $this->language->get('products_total'),
            ...
            if($data['discount_total']) {
                $totals[] = [
                    'code' => 'special',
                    'title' => $this->language->get('sale_price'),
        ...
        foreach ($totals as $total) {
                ...
                if($total['code'] == 'total'){
                    ...
                    $data['end_total'] = $this->currency->format($total['value'], $this->session->data['currency']);
                    continue;
                }
                ...
                if($total['code'] == 'shipping'){
                    ...
                    if($shipping_method['code'] == 'pickup.pickup'){
                        $total['value'] =  $this->language->get('delivery_free');
                    }else{
//                        $total['value'] = $this->language->get('delivery_carrier_tariffs');
                        $total['value'] =  $this->language->get('delivery_free');
                    }
                }
            ...
            $data['totals'][] = array(
                'title' => $total['title'],
                'text'  => is_numeric($total['value'])
                    ? $this->currency->format($total['value'], $this->session->data['currency'])
                    : $total['value']
            );
        ...
        if(isset($this->request->get['gettotal']) && $this->request->get['gettotal'] == 'y') {

            $this->response->addHeader('Content-Type: application/json');
            $this->response->setOutput(json_encode([
                'total' => $data['end_total']
            ]));
            return;
        }
...
//                $order_data['lastname']          = $customer_info['lastname'];
    ...
    public function sendUserToEsputnik($name = '', $lastname = '', $email = '', $phone = ''){

        $curl = curl_init();
        curl_setopt_array($curl, [
            CURLOPT_URL => "https://esputnik.com/api/v1/contacts",
            ...
            CURLOPT_POSTFIELDS => json_encode([
                ...
                'contacts' => [
                    [
                        ...
                        'fields' => [
                            [
                                'id' => 257717, // Додаткове поле Промокод
                                'value' => 'Promo1235'
                            ]
                        ]*/
                    ]
                ...
                /*'customFieldsIDs' => [
                    257717 // Додаткове поле Промокод
                ],*/
                'eventKeyForNewContacts' => 'newContactFtomSite'
            ]),
            CURLOPT_HTTPHEADER => [
                "accept: application/json; charset=UTF-8",
                "authorization: Basic MThCQkM5QTc2QTc1OTI2RjQwNzIzRkZFNjYzQzI1MDA6MThCQkM5QTc2QTc1OTI2RjQwNzIzRkZFNjYzQzI1MDA=",
