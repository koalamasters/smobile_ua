				...
				foreach ($add_products as $product_id) {
					...
					$recommended = $this->model_catalog_product->getProductRelated($product_id);
						...
						foreach ($recommended as $result) {
							...
							$this->_templateData['product_related_cart'][$result['product_id']] = array(
...
if ($this->config->get('remarketing_status') && !$this->model_tool_remarketing->isBot()) {
	...
	if ($this->config->get('remarketing_ecommerce_ga4_status')) {
        ...
        foreach ($totals as &$total){
            ...
            if($total['value'] == 0 && $total['code'] == 'shipping'){

                if(strpos($total['title'],'пошт') !== false){
                    $total['text'] = $this->language->get('delivery_info');
                }
