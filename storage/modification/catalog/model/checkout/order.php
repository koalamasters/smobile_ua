...
class ModelCheckoutOrder extends Model {
	  ...
	  $this->model_tool_remarketing->getOrderRemarketing($order_id);
			...
			if ($send_ntf == 1) { 
				...
				if($this->model_extension_module_ex_pak->load()){
					$this->model_extension_module_ex_pak->addComplectOrder($order_id, $this->cart->getProducts());
				}
    ...
    public function deleteOrder($order_id) {
      ...
      $this->db->query("DELETE FROM " . DB_PREFIX . "salesagent_temporder WHERE order_id = '" . (int)$order_id. "'");
      $this->db->query("DELETE FROM " . DB_PREFIX . "salesagent_transaction WHERE order_id = '" . (int)$order_id . "'");
    ...
    public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
        ...
        if ($order_info) {
                        ...
                        //Flowy Tracking - Saving order to generate code
                            if(!empty($order_status_id) && $this->FTMaster)
                                $this->DataLayer->set_data('order_id', $order_id);
            ...
            if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				$this->load->model('extension/module/ex_pak');
				if($this->model_extension_module_ex_pak->load()){
					$this->model_extension_module_ex_pak->processComplectOrder($order_id, 'minus');
				}
		...
		if ($this->config->get('remarketing_status') && !$this->model_tool_remarketing->isBot()) {
            ...
            if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && !in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				$this->load->model('extension/module/ex_pak');
				if($this->model_extension_module_ex_pak->load()){
					$this->model_extension_module_ex_pak->processComplectOrder($order_id, 'plus');
				}
