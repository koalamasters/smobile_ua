...
class ModelCatalogProduct extends Model {
    ...
    public function getProductAttributesUa($product_id) {
        $product_attribute_group_data = array();

        $product_attribute_group_query = $this->db->query("SELECT ag.attribute_group_id, agd.name FROM " . DB_PREFIX . "product_attribute pa LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_group ag ON (a.attribute_group_id = ag.attribute_group_id) LEFT JOIN " . DB_PREFIX . "attribute_group_description agd ON (ag.attribute_group_id = agd.attribute_group_id) WHERE pa.product_id = '" . (int)$product_id . "' AND agd.language_id = '3' GROUP BY ag.attribute_group_id ORDER BY ag.sort_order, agd.name");

        foreach ($product_attribute_group_query->rows as $product_attribute_group) {
            $product_attribute_data = array();

            $product_attribute_query = $this->db->query("SELECT a.attribute_id, ad.name, pa.text FROM " . DB_PREFIX . "product_attribute pa LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_description ad ON (a.attribute_id = ad.attribute_id) WHERE pa.product_id = '" . (int)$product_id . "' AND a.attribute_group_id = '" . (int)$product_attribute_group['attribute_group_id'] . "' AND ad.language_id = '3' AND pa.language_id = '3' ORDER BY a.sort_order, ad.name");
	...
	public function getUrlsWithLang($product_id){
		if($product_id > 0){
    ...
    // OCFilter end
        ...
        $query = $this->db->query($sql);
        if(isset($query->row['attr_tempalte_id'])) {
    ...
    public function getBaseAttrIds($attr_cat_id){
        $sql = "SELECT DISTINCT attribute_id 
FROM `oc_attribute_category_value` 
WHERE attr_cat_id = $attr_cat_id and base_attr = 1;";
    ...
    public function getNewStatus($product_id){
        $sql = "SELECT * FROM oc_product_new WHERE product_id = $product_id";
    ...
    public function getDateCreate($product_id){
        $sql = "SELECT date_added from oc_product WHERE product_id = $product_id";
    ...
    public function getInfoAboutlang($query){
    ...
    public function getActiveProductsCountByCategoryId($category_id) {
        $query = $this->db->query("
