...
class ModelCatalogCategory extends Model
{
    ...
    public function getCategoryUa($category_id) {
        $query = $this->db->query("
SELECT DISTINCT * FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id) WHERE c.category_id = '" . (int)$category_id . "' AND cd.language_id = '" . 3 . "' AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND c.status = '1'");
	...
	public function getCategoryUrlsWithLang($category_id)
	{
		if ($category_id > 0) {
			$query = $this->db->query("SELECT * FROM oc_seo_url WHERE query LIKE 'category_id=" . (int) $category_id . "%'");
